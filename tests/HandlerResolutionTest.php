...
use PhpSoftBox\CliApp\Tests\Support\DaemonHandlerClass;
use PhpSoftBox\CliApp\Tests\Support\DependentHandler;
use PhpSoftBox\CliApp\Tests\Support\FailingDaemonHandlerClass;
...
final class HandlerResolutionTest extends TestCase
{
    ...
    /**
     * Проверяет запуск daemon-команды через runAsDaemon().
     */
    #[Test]
    public function testRunsDaemonHandlerWhenCommandMarkedAsDaemon(): void
    {
        DaemonHandlerClass::$called = false;
        ...
        $registry->register(Command::define(
            ...
            signature: [],
            handler: DaemonHandlerClass::class,
            asDaemon: true,
        ));

        $resp = $app->runner()->run('daemon', []);

        self::assertSame(Response::SUCCESS, $resp->code);
        self::assertTrue(DaemonHandlerClass::$called);
        self::assertTrue($registry->get('daemon')?->asDaemon);
    }

    /**
     * Проверяет преобразование ожидаемой ошибки старта daemon-команды в Response.
     */
    #[Test]
    public function testDaemonStartupExceptionReturnsResponse(): void
    {
        $registry = new InMemoryCommandRegistry(withDefaultCommands: false);
        ...
        $registry->register(Command::define(
            name: 'daemon-fail',
            description: 'Failing daemon command',
            signature: [],
            handler: FailingDaemonHandlerClass::class,
            asDaemon: true,
        ));

        $resp = $app->runner()->run('daemon-fail', []);
